/* Ludo of Middle-earth — isometric parchment board rendered as inline SVG.
...
export function render({ game, state, els, E, action, titlebar, logBox, sound }) {
  ...
  const s = state.ludo || (state.ludo = { selectedUnitId: null, panel: null });
  ...
  els.gameStage.append(titlebar('Ludo of Middle-earth', status), root);
}
...
function buildTopBar({ game, you, E, doAction }) {
  const bar = E('div', 'ludo-topbar');
  bar.append(E('div', 'ludo-clock', `⏱ ${elapsedLabel(game.elapsedMs)}`));
  if (you) {
    bar.append(E('div', 'ludo-gold', `💰 ${you.gold}g`));
    const enemies = game.players.filter((p) => p.id !== you.id && !p.eliminated);
    const targetBtn = E('button', 'ludo-target');
...
function buildBoard({ game, you, s, E }) {
  const wrap = E('div', 'ludo-board-wrap');
  ...
  const pad = 14;
  const svg = svgEl('svg', { viewBox: `${minX - pad} ${minY - pad} ${maxX - minX + pad * 2} ${maxY - minY + pad * 2}`, class: 'ludo-svg' });
  const landLayer = svgEl('g', { class: 'ludo-land' });
  const homeLayer = svgEl('g', { class: 'ludo-home' });
  const castleLayer = svgEl('g', { class: 'ludo-castles' });
  const unitLayer = svgEl('g', { class: 'ludo-units' });
  svg.append(landLayer, homeLayer, castleLayer, unitLayer);

  // Siege square
  const sc = { x: isoX(CENTER.gx, CENTER.gy), y: isoY(CENTER.gx, CENTER.gy) };
  landLayer.append(svgEl('polygon', { points: diamond(sc.x, sc.y, TILE_W * 1.5, TILE_H * 1.5), class: 'ludo-siege-tile' }));
  const siegeLabel = svgEl('text', { x: sc.x, y: sc.y + 3, 'text-anchor': 'middle', class: 'ludo-siege-label' });
  ...
  FACTIONS.forEach((f, idx) => {
    for (let step = 0; step < HOME_STEPS; step += 1) {
      ...
      const poly = svgEl('polygon', { points: diamond(x, y, TILE_W * 0.85, TILE_H * 0.85), class: 'ludo-home-tile' });
  ...
  for (let i = 0; i < PATH_LENGTH; i += 1) {
    ...
    const poly = svgEl('polygon', { points: diamond(x, y, TILE_W, TILE_H), class: `ludo-tile ludo-tile-${info.role}` });
    ...
    if (info.role === 'defence' || info.role === 'attack') {
      const glyph = svgEl('text', { x, y: y + 3, 'text-anchor': 'middle', class: 'ludo-tile-glyph' });
  ...
  FACTIONS.forEach((f, idx) => {
    ...
    const group = svgEl('g', { class: `ludo-castle${p && p.eliminated ? ' fallen' : ''}` });
    ...
    group.append(svgEl('polygon', { points: diamond(x, y, towerW, towerH * 0.55), class: 'ludo-castle-base' }));
    const tower = svgEl('polygon', { points: `${x - towerW * 0.28},${y} ${x + towerW * 0.28},${y} ${x + towerW * 0.2},${y - towerH} ${x - towerW * 0.2},${y - towerH}`, class: 'ludo-castle-tower' });
    group.append(tower);
    const flag = svgEl('text', { x, y: y - towerH - 6, 'text-anchor': 'middle', class: 'ludo-castle-flag' });
    ...
    if (p) {
      ...
      group.append(svgEl('rect', { x: x - barW / 2, y: y - towerH - 20, width: barW, height: 5, class: 'ludo-hp-back' }));
      group.append(svgEl('rect', { x: x - barW / 2, y: y - towerH - 20, width: Math.max(0, barW * hpPct), height: 5, class: 'ludo-hp-fill' }));
      const tierLabel = svgEl('text', { x, y: y - towerH - 26, 'text-anchor': 'middle', class: 'ludo-tier-label' });
      tierLabel.textContent = '★'.repeat(p.turretTier) + '☆'.repeat(3 - p.turretTier);
      group.append(tierLabel);
      const nameLabel = svgEl('text', { x, y: y + towerH * 0.55 + 14, 'text-anchor': 'middle', class: 'ludo-castle-name' });
...
function buildUnitToken({ p, u, x, y, s, you }) {
  ...
  const g = svgEl('g', { class: `ludo-unit${s.selectedUnitId === u.id ? ' selected' : ''}${p.isYou ? ' mine' : ''}` });
  g.style.setProperty('--faction-color', meta.color);
  g.style.cursor = 'pointer';
  const r = 8;
  if (u.shielded) g.append(svgEl('circle', { cx: x, cy: y, r: r + 4, class: 'ludo-unit-shield' }));
  g.append(svgEl('circle', { cx: x, cy: y, r, class: 'ludo-unit-body' }));
  const hpPct = Math.max(0, u.hp / u.maxHp);
  const circumference = 2 * Math.PI * (r + 1.5);
  const ring = svgEl('circle', {
    cx: x, cy: y, r: r + 1.5, class: 'ludo-unit-hp',
    'stroke-dasharray': `${circumference * hpPct} ${circumference}`
  });
  g.append(ring);
  const glyph = svgEl('text', { x, y: y + 3, 'text-anchor': 'middle', class: 'ludo-unit-glyph' });
...
function buildStatusBar({ you, E }) {
  ...
  const bar = E('div', 'ludo-statusbar');
  bar.style.setProperty('--faction-color', meta.color);
  bar.append(E('span', 'ludo-status-icon', meta.icon));
  bar.append(E('span', 'ludo-status-name', `${meta.name} · ${meta.turretName} Lv${you.turretTier}`));
  const hpWrap = E('div', 'ludo-status-hp');
  const hpFill = E('div', 'ludo-status-hp-fill');
  hpFill.style.width = `${Math.max(0, Math.min(100, (you.castleHp / you.castleMaxHp) * 100))}%`;
  hpWrap.append(hpFill);
  bar.append(hpWrap, E('span', 'ludo-status-hp-text', `${Math.max(0, Math.round(you.castleHp))}/${you.castleMaxHp}`));
  return bar;
}
...
function buildTray({ game, you, s, canAct, E, doAction }) {
  const tray = E('div', 'ludo-tray');
  if (!you) { tray.append(E('div', 'ludo-tray-hint', 'Toeschouwer.')); return tray; }
  if (!canAct) {
    tray.append(E('div', 'ludo-tray-hint', you.eliminated ? 'Je kasteel is gevallen.' : (game.resultText || 'Beleg afgelopen.')));
    return tray;
  }

  const now = Date.now();
  const cooling = !you.rollPending && now < you.nextRollAt;
  const grid = E('div', 'ludo-actions');

  // ACTIE 1 — troepen plaatsen
  const btn1 = E('button', 'ludo-actbtn');
  btn1.type = 'button';
  btn1.disabled = cooling;
  if (you.rollPending) {
    btn1.append(E('span', 'ludo-actbtn-icon', '🎲'), E('span', 'ludo-actbtn-label', `Worp: ${you.lastRoll}`));
    btn1.disabled = you.lastRoll !== 6;
  } else {
    btn1.append(E('span', 'ludo-actbtn-icon', '🎲'), E('span', 'ludo-actbtn-label', 'Worp'));
  }
  btn1.onclick = () => {
  ...
  const btn2 = E('button', 'ludo-actbtn');
  btn2.type = 'button';
  btn2.append(E('span', 'ludo-actbtn-icon', '🏰'), E('span', 'ludo-actbtn-label', 'Versterk'));
  btn2.onclick = () => { s.panel = s.panel === 'reinforce' ? null : 'reinforce'; };
  grid.append(btn2);

  // ACTIE 3 — verplaats troepen
  const btn3 = E('button', 'ludo-actbtn');
  btn3.type = 'button';
  btn3.disabled = !you.rollPending;
  btn3.append(E('span', 'ludo-actbtn-icon', '🏃'), E('span', 'ludo-actbtn-label', you.rollPending ? `Verplaats ${you.lastRoll}` : 'Verplaats'));
  ...
  const btn4 = E('button', 'ludo-actbtn');
  btn4.type = 'button';
  const label4 = role === 'defence' ? 'Schild' : role === 'attack' ? 'Belegeren' : 'Actie';
  btn4.append(E('span', 'ludo-actbtn-icon', role === 'defence' ? '🛡' : role === 'attack' ? '🎯' : '✋'), E('span', 'ludo-actbtn-label', label4));
  ...
  if (s.panel === 'deploy' && you.rollPending && you.lastRoll === 6) {
    ...
    tray.append(E('div', 'ludo-tray-hint', secsLeft > 0 ? `Volgende worp over ${secsLeft}s.` : 'Klaar om te rollen.'));
  } else if (you.lastRoll !== 6) {
    tray.append(E('div', 'ludo-tray-hint', 'Tik een eenheid aan en verplaats haar met deze worp.'));
  }
...
function buildDeployPanel({ you, E, doAction }) {
  const panel = E('div', 'ludo-panel');
  UNIT_ORDER.forEach((cls) => {
    const meta = UNIT_META[cls];
    const btn = E('button', 'ludo-panel-btn');
    btn.type = 'button';
    btn.disabled = you.gold < meta.cost;
    btn.append(
      E('span', 'ludo-panel-icon', meta.icon),
      E('span', 'ludo-panel-name', meta.names[you.faction]),
      E('span', 'ludo-panel-cost', `${meta.cost}g`)
    );
...
function buildReinforcePanel({ you, E, doAction }) {
  const panel = E('div', 'ludo-panel');
  const repair = E('button', 'ludo-panel-btn');
  repair.type = 'button';
  repair.disabled = you.gold < 200 || you.castleHp >= you.castleMaxHp;
  repair.append(E('span', 'ludo-panel-icon', '🧱'), E('span', 'ludo-panel-name', 'Herstel +250 HP'), E('span', 'ludo-panel-cost', '200g'));
  repair.onclick = () => doAction('repair');
  panel.append(repair);

  const upgrade = E('button', 'ludo-panel-btn');
  upgrade.type = 'button';
  upgrade.disabled = you.gold < 150 || you.turretTier >= 3;
  upgrade.append(E('span', 'ludo-panel-icon', '🔧'), E('span', 'ludo-panel-name', 'Toren upgraden'), E('span', 'ludo-panel-cost', '150g'));
