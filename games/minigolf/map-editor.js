...
async function showMinigolfEditor(){
  if(session.room)return toast('Verlaat eerst de game.');hideMainViews();els.minigolfEditorView.classList.remove('hidden');els.rulesButton.classList.add('hidden');const ed=ensureMapEditor();syncEditorMetaFromMap();renderMapEditorCanvas();renderMapInspector();await loadCustomMapLibrary();
}
