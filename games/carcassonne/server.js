...
function serialize(game,requesterId,connected){const current=game.players[game.turnIndex],mine=current?.id===requesterId;return {kind:meta.key,phase:game.phase,gameOver:game.gameOver,resultText:game.resultText,turnPlayerId:game.gameOver?null:current?.id,tilesRemaining:game.deck.length+(game.currentTile?1:0),discarded:game.discarded,currentTile:mine&&game.phase==='place'?serializeTile(game.currentTile):null,validPlacements:mine&&game.phase==='place'?game.validPlacements:[],lastPlaced:game.lastPlaced?{x:game.lastPlaced.x,y:game.lastPlaced.y}:null,lastPlayed:game.lastPlayed?{...game.lastPlayed}:null,finalScoreBreakdown:game.gameOver?(game.finalScoreBreakdown||[]):undefined,meepleChoices:mine&&game.phase==='meeple'&&game.lastPlaced?meepleChoices(game,game.lastPlaced):[],board:[...game.board.values()].map(e=>({x:e.x,y:e.y,tile:serializeTile(e.tile)})),meeples:game.meeples,players:game.players.map(p=>({...p,connected:p.isNpc||connected.get(p.id)})),log:game.log.slice(0,20)}}
...
module.exports={meta,createGame,handleAction,serialize,tick,rotateTile,canPlaceAt,placementsFor,feature,fieldFeature,meepleChoices,scoreEnd,results};
