...
function makeDeck(tileCount=72){const deck=[];let index=0;for(const [type,count] of DECK_SPEC)for(let i=0;i<count;i+=1)deck.push(cloneTile(type,index++));return shuffle(deck).slice(0,[18,36,72].includes(Number(tileCount))?Number(tileCount)-1:71)}
...
function createGame(roomPlayers,options={}){
  const tileCount=[18,36,72].includes(Number(options.tileCount))?Number(options.tileCount):72;const start=cloneTile('cityCapRoad','start');start.id='start';const board=new Map([[posKey(0,0),{x:0,y:0,tile:start}]]);const game={gameKey:meta.key,tileCount,players:roomPlayers.map((p,i)=>({id:p.id,name:p.name,isNpc:p.isNpc,score:0,meeples:7,color:COLORS[i]})),board,deck:makeDeck(tileCount),currentTile:null,validPlacements:[],lastPlaced:null,lastPlayed:null,meeples:[],scored:new Set(),turnIndex:0,phase:'place',discarded:0,gameOver:false,resultText:'',log:[],nextNpcAt:0};drawNext(game);scheduleNpc(game,700);return game
}
