...
export function onRoomReset(){scorePopupOpen=false}
...
function renderHartenjagen(room,game) {
  ...
  const scoreButton=E('button','hearts-score-button','Score');scoreButton.type='button';scoreButton.onclick=()=>{scorePopupOpen=true;renderGame(state.room)};els.gameStage.append(scoreButton);
  if(scorePopupOpen)openScorePopup(game);
}
...
function openScorePopup(game){
  ...
  const dismiss=()=>{scorePopupOpen=false;backdrop.remove()};close.onclick=dismiss;backdrop.onclick=(event)=>{if(event.target===backdrop)dismiss()};
