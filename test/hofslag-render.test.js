...
test('Hofslag gebruikt eigen renderpad vóór generieke player-strip',()=>{
 ...
 const strip=app.indexOf("if(!['minigolf','blackjack','pesten','presidenten'].includes(game.kind))els.gameStage.append(renderGamePlayerStrip(room,game));");
 assert.ok(special>0);
...
test('Blackjack, Pesten en Presidenten verbergen de generieke player-strip',()=>{
 const app=fs.readFileSync(gameUiPath,'utf8');
 assert.match(app,/!\['minigolf','blackjack','pesten','presidenten'\]\.includes\(game\.kind\)/);
