...
test('missed releases are grouped instead of shown as version history', () => {
  ...
  assert.deepEqual(changes.games,['Kingdomino','Cascadia','Isle of Skye','The Deep Bleu C','CycClub','Ragnarok','Bakkermans Jones','Ludo of Middle-earth']);
...
test('guest with an older seen version gets only relevant grouped changes', () => {
  ...
  assert.deepEqual(payload.changes.games,['Isle of Skye','The Deep Bleu C','CycClub','Ragnarok','Bakkermans Jones','Ludo of Middle-earth']);
  assert.ok(payload.changes.features.length>=44);
  assert.ok(payload.changes.improvements.length>=89);
  ...
  assert.ok(payload.changes.features.some((item)=>item.includes('Ludo of Middle-earth')));
