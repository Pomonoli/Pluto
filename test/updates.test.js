...
test('missed releases are grouped instead of shown as version history', () => {
  ...
  assert.deepEqual(changes.games,['Kingdomino','Cascadia','Isle of Skye','The Deep Bleu C','CycClub','Ragnarok','Bakkermans Jones','Elements Arena','Lutro']);
...
test('guest with an older seen version gets only relevant grouped changes', () => {
  ...
  assert.deepEqual(payload.changes.games,['Isle of Skye','The Deep Bleu C','CycClub','Ragnarok','Bakkermans Jones','Elements Arena','Lutro']);
  ...
  assert.ok(payload.changes.features.some((item)=>item.includes('Lutro')));
